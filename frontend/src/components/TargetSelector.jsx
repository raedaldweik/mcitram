      ...
      {items.map(item => {
        ...
        return (
          <button key={item.id} onClick={() => pick({ ...item, type })}
            className="w-full text-left px-3 py-2 transition-all hover:bg-[rgba(7,102,209,0.06)]"
            style={isActive ? { background: 'rgba(7,102,209,0.10)', borderInlineStart: '3px solid var(--gold)' } : { borderInlineStart: '3px solid transparent' }}>
      ...
      <button onClick={() => setOpen(!open)}
        ...
        {selected && (
          <span className="text-[8.5px] font-bold tracking-wider uppercase px-1.5 py-0.5 rounded shrink-0"
            style={{ background: 'rgba(7,102,209,0.10)', color: 'var(--gold)' }}>
            {selected.type}
          </span>
        )}
